# 知识网络 Knowledge Network

知识网络是 `VIRTURA-SpacePort` 的主宿主内容之一。

它不是一个资料堆，也不是只给内部成员看的笔记区，而是一个可以反复回来使用的学习与实践入口：帮助创作者、研究者和协作者把分散的经验、案例和方法组织成可进入的路径。

从空间计算、音画互动、网页发布，到学习路线、现场表演和团队方法，这里当前已经整理出 10 个知识领域与一组可直接进入的学习路径。

## 这里现在已经有什么

- 10 个知识领域导航
- `learning-paths` 形式的最短路径入口
- `our-theory` 及其下的团队方法框架
- 一批仍在持续补充的案例索引、术语入口和工作流整理

## 第一次怎么用

按照你的目的，可以直接走不同入口:

- 想快速理解整体结构: 先看“两个坐标系”
- 想从兴趣主题进入: 直接看 10 个知识领域
- 想从学习路径进入: 优先看“学习路线”
- 想看团队自己的方法论: 进入 `our-theory`
- 想帮助建设知识网络: 先看“如何参与建设”

如果你是第一次来，真的不用从头读到尾。选一条路线就够了。

## 两个坐标系

这个知识网络同时有两种组织方式，它们不是冲突关系:

1. 主题坐标: 10 个知识领域
   适合按兴趣、技能和工作需求进入
2. 路径坐标: 学习路线与最短路径
   适合从零到一、按阶段推进

简单说:

- 10 个知识领域回答“你要学什么”
- 学习路线回答“你先从哪里开始”

## 这个系统最想帮你解决什么

知识网络当前最强调的是:

- 最短路径: 尽快跨过入门门槛
- 多种实现方法: 不把单一路线误写成唯一答案
- 困境预设: 提前说明常见阻塞和替代方案
- 交叉学科: 允许空间、声音、网页、AI、演出互相连接
- 与真实项目相连: 尽量让知识回到作品、工具和发布中

## 三条推荐入口

- 想尽快做出第一个东西：看 [learning-paths](./learning-paths/README.md)
- 想理解团队自己的方法框架：看 [our-theory](./our-theory/README.md)
- 想按具体技能切入：从下面 10 个知识领域里选一个

## 10个知识领域大类

如果你不想直接扎进长目录，先从下面的领域导航开始：

| 领域 | 更适合谁先看 | 入口 |
|---|---|---|
| 空间计算 / 设备 | 想尽快做出第一个设备端 Demo 的人 | [spatial-computing](./spatial-computing/README.md) |
| Blender 实时引擎 | 想把 Blender 当成可复用的实时创作底座的人 | [blender-realtime](./blender-realtime/README.md) |
| 音画互动 | 想把声音和视觉真正连起来的人 | [audio-visual](./audio-visual/README.md) |
| 3D 重建 / 可视化 | 想从扫描、生成到高斯点云的人 | [3d-reconstruction](./3d-reconstruction/README.md) |
| 网页端 / 发布 | 想把内容迁到网页端和公开入口的人 | [web-publishing](./web-publishing/README.md) |
| 生产管线 | 想看跨软件联动和工程化组织的人 | [production-pipeline](./production-pipeline/README.md) |
| 角色体系 | 想补导演、叙事、分镜和创作软技能的人 | [role-system](./role-system/README.md) |
| 学习路线 | 想从零到一按路径推进的人 | [learning-paths](./learning-paths/README.md) |
| 现场表演 / 剧场艺术 | 想把空间感知带回演出和现场的人 | [live-performance](./live-performance/README.md) |
| 我们的理论 / 后日谈 | 想直接读团队自己的方法和反思的人 | [our-theory](./our-theory/README.md) |

## Recommended First Routes

如果你是第一次进入，这里更推荐直接按目的选路线，而不是从头读到尾：

- 想尽快做出第一个作品：先看 [learning-paths](./learning-paths/README.md)
- 想做视觉与声音联动：先看 [audio-visual](./audio-visual/README.md)
- 想把内容迁到网页端：先看 [web-publishing](./web-publishing/README.md)
- 想理解团队自己的方法论：先看 [our-theory](./our-theory/README.md)

## 如何参与建设

如果你想帮助建设这个知识网络，优先从这些低摩擦入口开始:

- 补一个主题入口页
- 整理一个案例索引
- 帮某个模块补“最短路径”
- 把项目复盘中的方法抽成知识条目
- 帮现有页面做结构整理和导览优化

---

## 知识领域详解

下面这部分更接近长期资料目录。

如果你只是第一次进入，不一定需要从这里开始通读；它更适合在你已经选定一个方向之后，再往下钻。

### 1. 空间计算/设备

目标：帮助零基础用户在30分钟内发布第一个Demo

- **Apple Vision Pro**
  - 最短路径：新手指南（30分钟内）
  - 多种实现方法：手势/眼动/语音
  - 官方资源筛选：2024-2025最新
  - 实战案例：180°视频→Blender处理→导出双目→App独立运行
  - 困境预设 + 解决方案

- **Meta Quest**
  - 最短路径
  - 多种实现方法
  - 官方资源
  - 困境预设 + 解决方案

[进入空间计算/设备 →](./spatial-computing/README.md)

---

### 2. Blender实时引擎

目标：把Blender当实时引擎用，Eevee渲染快

- **Eevee快速渲染**
  - 最短路径（5分钟快速渲染）
  - 多种实现方法：Eevee/Cycles/eevee-next

- **几何节点**
  - 最短路径
  - 多种实现方法：音频Value输入/BPM驱动/MIDI信号

- **模拟节点**
  - 最短路径
  - 多种实现方法：物理/流体/cloth
  - 与音画互动交叉 → 特色内容

- **光影/材质基础**

[进入Blender实时引擎 →](./blender-realtime/README.md)

---

### 3. 音画互动

目标：音频驱动视觉，以及与几何节点的交叉使用

- **音频分析**
  - 最短路径：快速频谱分析
  - 多种方法：BPM/MIDI/频谱/Value

- **音频驱动视觉**
  - 频谱驱动/节拍驱动/频率驱动

- **OSC/时间码同步**
  - 最短路径
  - 多种方法：OSC/NDI/LTC/SMPTE

[进入音画互动 →](./audio-visual/README.md)

---

### 4. 3D重建/可视化

目标：从扫描到AI生成到高斯点云

- **物体扫描**
  - 最短路径：iPhone LiDAR扫描
  - 多种方法：LiDAR/Photogrammetry/AI

- **AI生成3D**
  - 最短路径：图片→3D
  - 多种方法：Meshy/Tripo/Point-E
  - AI效果对比：中等偏下vs精益求精

- **高斯点云**
  - 最短路径：图片→Gaussian Splatting
  - 多种方法：Apple Shapr/ComfyUI/COLMAP

[进入3D重建/可视化 →](./3d-reconstruction/README.md)

---

### 5. 网页端/发布

目标：快速网页Demo，性能优化

- **Spline**
  - 最短路径：快速网页Demo
  - 能做什么/不能做什么

- **Three.js/Babylon.js**
  - 最短路径
  - Blender导出工作流

- **网页端性能优化**
  - 烘焙技巧：模型/贴图→单张纹理
  - 10万面/20万面优化方案

[进入网页端/发布 →](./web-publishing/README.md)

---

### 6. 生产管线

目标：完整的扫描→AI→高斯流程，工程化管理

- **扫描→AI→高斯流程总览**
- **实时渲染工作流（Blender→网页）**
- **跨软件联动**
  - Blender→Unity
  - Blender→SceneForge
  - Blender→LiveForge
- **工程化管理**
  - 文件管理
  - 版本控制
  - 资产整理

[进入生产管线 →](./production-pipeline/README.md)

---

### 7. 角色体系

目标：导演、叙事、分镜头等软技能

- **导演思维**
- **叙事设计**
- **分镜头脚本**
- **服务设计**
- **刻意练习科目**

[进入角色体系 →](./role-system/README.md)

---

### 8. 学习路线

目标：最短路径 + 进阶路径 + 困境预设

- **最短路径（零基础→第一个作品）**
  - 空间计算：1天
  - Blender：3天
  - 音画互动：1天
  - 生产管线：3天

- **进阶路径**
- **困境预设 + 解决方案**

[进入学习路线 →](./learning-paths/README.md)

---

### 9. 现场表演/剧场艺术

目标：收集剧场现场表演相关案例，具身理论，以及空间感知在演出中的应用

- **具身理论**：心理学、具身认知（与VR相关）
- **经典案例**：rhizomatiks syn（2022年11月Tokyo）
- **更多案例**：多媒体投影舞剧等
- **2026年论坛**：设计理论、空间叙事

[进入现场表演/剧场艺术 →](./live-performance/README.md)

---

### 10. 我们的理论/后日谈

目标：记录我们的心得，体会、总结，以及对空间感知体验的独特诠释

- **我们的心得体会**
- **项目复盘**
- **技术心得**
- **设计思考**

[进入我们的理论/后日谈 →](./our-theory/README.md)

---

## 当前特点

### 1. 知识领域相互关联
不是线性排列，而是10个相互关联的知识领域

### 2. 最短路径
帮助零基础用户快速跨越鸿沟

### 3. 多种实现方法
每个模块列出2-3种实现方法

### 4. 困境预设
提前告知可能遇到的问题和解决方案

### 5. 官方资源筛选
筛选最新、最有效的官方文档和社区教程

### 6. 交叉方向
几何节点与音画互动的交叉使用

---

## 贡献与反馈

知识网络当前仍在持续整理，欢迎从结构、案例和入口层面补充：

- **问题反馈**：使用反馈模板
- **经验贡献**：欢迎提交你的实操经验、案例分析
- **内容建议**：提出你希望补充的知识点

---

## 版本更新日志

### v2.0.1 (2026-03-01)
- 新增林中茶屋相关入口
- 知识网络重构为**10个**知识领域大类
- 新增现场表演/剧场艺术大类
- 新增我们的理论/后日谈大类
- 强调知识相互关联（交叉学科）
- 新增困境预设机制
- 新增交叉方向标注

### v2.0.0 (2026-02-25)
- 重构为8个知识领域大类
- 新增网页端/发布大类
- 新增角色体系大类
- 强调知识相互关联（交叉学科）
- 新增困境预设机制
- 新增交叉方向标注

### v1.0.0 (2026-02-24)
- 初始版本发布

---

*知识网络 - 持续整理中的公开方法入口*
